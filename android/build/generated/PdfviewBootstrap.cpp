...
#define TAG "fr.squirrel.pdfview"

using namespace v8;

static Persistent<Object> bindingCache;

static void Pdfview_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Pdfview.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::PdfviewBindings::lookupGeneratedInit(
...
static void Pdfview_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Pdfview_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Pdfview_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::PdfviewBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PdfviewBinding = {
	"fr.squirrel.pdfview",
	Pdfview_init,
	Pdfview_dispose
};
...
Java_fr_squirrel_pdfview_PdfviewBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("fr.squirrel.pdfview", &PdfviewBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::PdfviewBindings::lookupGeneratedInit));
}
